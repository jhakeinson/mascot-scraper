...
async def login_to_website(username, password, page):
    ...
    try:
        ...
        logger.warning("Timeout while waiting after Sign In")
...
async def get_all_category_locators(
...
async def process_category_form(category_text, page):
    ...
    if await item_spec_locator.count() == 0:
        raise Exception(f"Cannot find item spec locator for {category_text}")
...
async def extract_form_data(page: Page) -> List[Field]:
    ...
    categories = await get_all_category_locators(category_dropdown_selector, page)
    ...
    for category in categories:
        ...
        count += 1
        logger.info(f"progress: {category_text} - {count}/{len(categories)}")
